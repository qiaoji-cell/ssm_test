<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%
String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() +
request.getContextPath() + "/";
%>
<!DOCTYPE html>
<html>
<head>
    <base href="<%=basePath%>">
    <meta charset="UTF-8">
    <title>后台系统-登陆</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta http-equiv="Access-Control-Allow-Origin" content="*">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <meta name="apple-mobile-web-app-status-bar-style" content="black">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="format-detection" content="telephone=no">
    <link rel="icon" href="/statics/images/purple_kindergarten.png">
    <link rel="stylesheet" href="statics/lib/layui-v2.6.3/css/layui.css" media="all">
    <!--[if lt IE 9]>
    <script src="https://cdn.staticfile.org/html5shiv/r29/html5.min.js"></script>
    <script src="https://cdn.staticfile.org/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->
    <style>
        html, body {width: 100%;height: 100%;overflow: hidden}
        /*body {background: #1E9FFF;}*/
        body{
            background:#3c40c6;
            /*background-image: url(/statics/images/stars-opacity.svg);*/
            background-image: url(/statics/images/night.gif);
        }
        body:after {content:'';background-repeat:no-repeat;background-size:cover;-webkit-filter:blur(3px);-moz-filter:blur(3px);-o-filter:blur(3px);-ms-filter:blur(3px);filter:blur(3px);position:absolute;top:0;left:0;right:0;bottom:0;z-index:-1;}
        .layui-container {width: 100%;height: 100%;overflow: hidden}
        .admin-login-background {width:360px;height:300px;position:absolute;left:50%;top:40%;margin-left:-180px;margin-top:-100px;}
        .logo-title {text-align:center;letter-spacing:2px;padding:14px 0;}
        .logo-title h1 {color:#3c40c6;font-size:25px;font-weight:bold;}
        .login-form {background-color:#fff;border:1px solid #fff;border-radius:3px;padding:14px 20px;box-shadow:0 0 8px #eeeeee;}
        .login-form .layui-form-item {position:relative;}
        .login-form .layui-form-item label {position:absolute;left:1px;top:1px;width:38px;line-height:36px;text-align:center;color:#d2d2d2;}
        .login-form .layui-form-item input {padding-left:36px;}
        .captcha {width:60%;display:inline-block;}
        .captcha-img {display:inline-block;width:34%;float:right;cursor:pointer}
        .captcha-img img {height:34px;border:1px solid #e6e6e6;height:36px;width:100%;}
        .layui-btn-normal {
            background-color: #3c40c6;
        }
    </style>
</head>
<body>
<div class="layui-container">
    <div class="admin-login-background">
        <div class="layui-form login-form">
            <form class="layui-form" lay-filter="loginForm">
                <div class="layui-form-item logo-title">
                    <h1>幼儿园管理系统登录</h1>
                </div>
                <div class="layui-form-item">
                    <label class="layui-icon layui-icon-username" ></label>
                    <input type="text" name="userCode" lay-verify="required|account" placeholder="用户名或者邮箱" autocomplete="off" class="layui-input" value="admin">
                </div>
                <div class="layui-form-item">
                    <label class="layui-icon layui-icon-password"></label>
                    <input type="password" name="password" lay-verify="required|password" placeholder="密码" autocomplete="off" class="layui-input" value="123456">
                </div>
                <div class="layui-form-item">
                    <label class="layui-icon layui-icon-vercode"></label>
                    <input type="text" name="captcha" lay-verify="required|captcha" placeholder="图形验证码" autocomplete="off" class="layui-input verification captcha" value="">
                    <div class="captcha-img">
                        <img id="captchaPic" src="/sys/user/captcha">
                    </div>
                </div>
                <div class="layui-form-item">
                    <input type="checkbox" name="rememberMe" value="true" lay-skin="primary" title="记住密码">
                </div>
                <div class="layui-form-item">
                    <button class="layui-btn layui-btn layui-btn-normal layui-btn-fluid" lay-submit lay-filter="login">登 入</button>
                </div>
            </form>
        </div>
    </div>
</div>
<script src="http://pv.sohu.com/cityjson?ie=utf-8"></script>
<script src="statics/lib/jquery-3.4.1/jquery-3.4.1.min.js" charset="utf-8"></script>
<script src="statics/lib/layui-v2.6.3/layui.js" charset="utf-8"></script>
<script src="statics/lib/jq-module/jquery.particleground.min.js" charset="utf-8"></script>
<script>
    layui.use(['form','layer'], function () {
        var form = layui.form,
            layer = layui.layer;
        let pathName = document.location.pathname;
        let index = pathName.substr(1).indexOf("/");
        let result = pathName.substr(0,index+1);
        //有cookie自动填充
        function getCookie(){
            $.get(result+"sys/user/autoLogin")
            .done((res)=>{
                if(res.data!=null){
                    console.log(res.data)
                    // let user = JSON.stringify(res.data);
                    let userCode = res.data.user_code;
                    let userPassword = res.data.userPassword;

                    // form.val("loginForm",{
                    //     "userCode":userCode,
                    //     "password":userPassword,
                    //     'rememberMe':true
                    // })
                    $("[name=userCode]").val(userCode);
                    $("[name=userPassword]").val(userPassword);
                    $("[name=rememberMe]").attr("checked",true);
                    form.render();
                }
            });
        }
        getCookie();

        $("#captchaPic").click(function (){
            $(this).attr("src","/sys/user/captcha?"+new Date().getTime());
        });
        // 登录过期的时候，跳出ifram框架
        if (top.location != self.location) top.location = self.location;

        // 粒子线条背景
        $(document).ready(function(){
            $('.layui-container').particleground({
                dotColor:'#4267d0',
                lineColor:'#4267d0'
            });
        });
        function sendLog(callBack){
            let ip = '';
            let address = '';
            if(typeof returnCitySN != 'undefined'){
                ip = returnCitySN["cip"];
                address = returnCitySN["cname"];
            }

            console.log(ip+"---"+address)
            let param = {};
            param.ip = ip;
            param.address = address;
            $.post("sys/log/add",param)
                .done((res)=>{
                    if(res.code!=3001){
                        layer.msg('登录成功！即将进入首页',{icon:6,offset:'100px'},function (){
                            callBack();
                        })
                    }
                })
        }

        //表单验证
        form.verify({
            account:function (value,item){
                if(!new RegExp("^[a-zA-Z0-9_\u4e00-\u9fa5\\s·]+$").test(value)){
                    return '用户名不能有特殊字符';
                }
                if(/(^\_)|(\__)|(\_+$)/.test(value)){
                    return '用户名首尾不能出现下划线\'_\'';
                }
                if(/^\d+\d+\d$/.test(value)){
                    return '用户名不能全为数字';
                }
            },
            password:[
                /^[\S]{6,12}$/,
                '密码必须6到12位，且不能出现空格'
            ],
            captcha:function (value,item){
                //false通过
                let res = false;
                //发送请求验证
                $.ajax({
                    url:"/sys/user/captcha",
                    type:"post",
                    dataType:"json",
                    async:false,
                    success:({data})=>{
                        if(data.toUpperCase()!=value.toString().toUpperCase()){
                            res = true;
                        }else{
                            res = false;
                        }
                    }
                })
                if(res){
                    return "验证码不正确";
                }
            }
        })

        // 进行登录操作
        form.on('submit(login)', function () {
            let param = form.val("loginForm");
            $.post("/sys/user/login",param)
            .done((res)=>{
                if(res.code==3001){
                    return layer.msg(res.msg);
                }
                sendLog(function (){
                    window.location = 'sys/user/index';
                });
            })
            return false;
        });
    });
</script>
</body>
</html>